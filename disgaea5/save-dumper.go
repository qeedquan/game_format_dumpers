...
func main() {
	...
	err = ioutil.WriteFile(flag.Arg(1), data, 0644)
	ck(err)
}
...
func usage() {
	fmt.Fprintln(os.Stderr, "Disgaea 4/5 Save Dumper")
...
func decode(src []byte) ([]byte, error) {
	const (
		...
		magic3 = "NIS_DISGAEA5_COMPLETE_SAVEDATA_"
		magic4 = "NIS_DISGAEA5_TRIAL_SAVEDATA_"
	)
	...
	if !bytes.HasPrefix(buf, []byte(magic1)) && !bytes.HasPrefix(buf, []byte(magic2)) &&
		!bytes.HasPrefix(buf, []byte(magic3)) && !bytes.HasPrefix(buf, []byte(magic4)) {
...
func (z *ykcmp) Encode(src []byte) ([]byte, error) {
	w := new(bytes.Buffer)

	var comp nipponichi.Compress
	enc, err := comp.Encode2(src)
	w.Write(enc)
...
func (z *ykcmp) Decode(src []byte) ([]byte, error) {
	...
	case 4:
		var comp nipponichi.Compress
		dst, err = comp.Decode2(src, math.MaxUint32)

	case 3: // TODO: 4 uses this
		fallthrough
	default:
		return nil, fmt.Errorf("ykcmp: unsupported compression type: %v", typ)
	}
