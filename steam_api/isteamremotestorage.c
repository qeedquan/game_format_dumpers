...
S_CLASSAPI bool S_CLASSCALLTYPE SteamRemoteStorage_IsCloudEnabledForAccount(SELF)
{
	bool enabled = false;
...
S_CLASSAPI bool S_CLASSCALLTYPE SteamRemoteStorage_IsCloudEnabledForApp(SELF)
{
	bool enabled = false;
	xlogf("%s(self = %p) -> %d\n", __func__, self, enabled);
	return enabled;
...
S_CLASSAPI bool S_CLASSCALLTYPE
SteamAPI_ISteamRemoteStorage_GetQuota(SELF, int32 *pnTotalBytes, int32 *puAvailableBytes)
{
