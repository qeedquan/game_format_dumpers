...
{
	xlogf("%s(self = %p, handlesOut = %p)", __func__, self, handlesOut);
	return 0;
}
...
{
	xlogf("%s(self = %p, pszActionSetName = %s)", __func__, self, pszActionSetName);
	return 0;
}
...
{
	xlogf("%s(self = %p, inputHandle = %llu, actionSetHandle = %llu)", __func__, self, inputHandle, actionSetHandle);
}

S_CLASSAPI InputActionSetHandle_t S_CLASSCALLTYPE
SteamAPI_ISteamInput_GetCurrentActionSet(SELF, InputHandle_t inputHandle)
{
	xlogf("%s(self = %p, inputHandle = %llu)", __func__, self, inputHandle);
	return 0;
}
...
{
	xlogf("%s(self = %p, inputHandle = %llu, actionSetHandle = %llu)", __func__, self, inputHandle, actionSetLayerHandle);
}
...
{
	xlogf("%s(self = %p, inputHandle = %llu, handlesOut = %p)", __func__, self, inputHandle, handlesOut);
	return 0;
}
...
{
	xlogf("%s(self = %p, pszActionName = %s)", __func__, self, pszActionName);
	return 0;
}
...
SteamAPI_ISteamInput_GetDigitalActionData(SELF, InputHandle_t inputHandle, InputDigitalActionHandle_t digitalActionHandle)
{
	...
	xlogf("%s(self = %p, inputHandle = %llu, digitalActionHandle = %llu)", __func__, self, inputHandle, digitalActionHandle);
	return data;
}
...
{
	xlogf("%s(self = %p, inputHandle = %llu, actionSetHandle = %llu, digitalActionHandle = %llu, originsOut = %p)", __func__, self, inputHandle, actionSetHandle, digitalActionHandle, originsOut);
	return 0;
}
...
SteamAPI_ISteamInput_GetAnalogActionData(SELF, InputHandle_t inputHandle, InputAnalogActionHandle_t analogActionHandle)
{
	...
	xlogf("%s(self = %p, inputHandle = %llu, analogActionHandle = %llu)", __func__, self, inputHandle, analogActionHandle);
	return data;
}
...
{
	xlogf("%s(self = %p, eOrigin = %d", __func__, self, eOrigin);
	return "";
}
...
{
	xlogf("%s(self = %p, inputHandle = %llu, eAction = %llu", __func__, self, inputHandle, eAction);
}
...
{
	xlogf("%s(self = %p, nIndex = %d", __func__, self, nIndex);
	return 0;
}
...
{
	xlogf("%s(self = %p, ulControllerHandle = %llu", __func__, self, ulControllerHandle);
	return 0;
}
...
{
	xlogf("%s(self = %p, inputHandle = %llu, pMajor = %p, pMinor = %p", __func__, self, inputHandle, pMajor, pMinor);
	*pMajor = 1;
	*pMinor = 0;
	return true;
}
